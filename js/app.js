...
const	seasonNames = ['Proloće', 'Leto', 'Jesen', 'Zima'],
		weekdaysNames = ["Nedelja", "Ponedeljak", "Utorak", "Sreda", "Četvrtak", "Petak", "Subota"],
		monthNames = ["Januar","Februar","Mart","April","Maj","Jun","Jul", "Avgust", "Septembar", "Oktobar", "Novembar", "Decembar"],
		pantryKey = '1f3a2c9e-cbc5-4c5b-b93f-5d0f48a83ce8',
		pantryBasket = 'food';
 ...
 */
function WeekdayAsString(dayIndex) {
    return weekdaysNames[dayIndex];
}

/*
 * Get current season first letter
 */
function getCurrentSeaoson() {
    ...
    if (today >= firstSpring && today < firstSummer) {
        return seasonNames[0][0].toLowerCase();
    } else if (today >= firstSummer && today < firstFall) {
        return seasonNames[1][0].toLowerCase();
    } else if (today >= firstFall && today < firstWinter) {
        return seasonNames[2][0].toLowerCase();
    } else if (today >= firstWinter || today < firstSpring) {
        return seasonNames[3][0].toLowerCase();
    }
}

/*
 * Filter food by current season
 */
function FilterBySeason( json, season ) {
	if( !season ) season =  getCurrentSeaoson();

	return json.filter(function(current) {

		return ( current.hasOwnProperty('cat') && current.cat.indexOf(season) > -1 );

	});
}

/*
 * Filter food without next key
 */
function FilterWithoutNext( json ) {

	return json.filter(function(current) {

		return ( !current.hasOwnProperty('next') || current.next === '' );
...
/*
 * Filter food by next key
 */
function FilterByNext( json, currentDay ) {

	if ( currentDay ) {

		return json.filter(function(current) {

			return ( current.hasOwnProperty('next') && currentDay.indexOf( current.next ) > -1 );
...
/*
 * Filter food by last key
 */
function FilterByLast( json, currentDay ) {

	if ( currentDay ) {

		return json.filter(function(current) {

			return ( current.hasOwnProperty('last') && currentDay.indexOf( current.last ) > -1 );
		...
		return json.filter(function(current) {
			...
			return ( current.hasOwnProperty('last') && current.last != '' );
...
/*
 * Get prev 3 days, today and next 3 days
 */
function GetDates(startDate, daysToAdd = 3) {
	if ( !startDate ) startDate = new Date();

    var currentSevenDays = [],
	...
	for(var i = 0; i <= daysToAdd; i++) {
        ...
        currentSevenDays.push(WeekdayAsString(currentDate.getDay()) + '.' + currentDate.getDate() + '.' + monthNames[month] + '.' + currentDate.getFullYear());
    }

	currentSevenDays = currentSevenDays.reverse();
...
/*
 * Create html for food wrapper
 */
function CreateFoodHtmlElement ( cat, icon, name, rating, edit = true ) {
	var food = '<div class="food" data-cat="'+cat+'"><div class="icon"><img data-src="'+iconUrl+iconNames[icon]+'"></div><div class="food-side"><div class="food-name"><span>'+name+'</span></div><div class="food-rating"><i data-star="'+rating+'"></i></div>';
...
/*
 * Fill food list with food
 */
function FillFoodListWithFood() {	
	var foodList = $('.section[data-cat="foods"] .food-list');
	for (let i = 0; i < json.foods.length; i++) {
		var	currentFood = json.foods[i],
			html = CreateFoodHtmlElement(currentFood.cat, currentFood.icon, currentFood.name, currentFood.rating)
 ...
 */
function RemoveWeekDayFromString( day ) {
	return day.split('.').slice(1).join('.')
}

/*
 * Generate food for the next day
 */
function GenerateFood() {
	
	var tempJson = json.foods,
		lastFood = FilterByLast(tempJson);
		
	for (let i = 0; i < lastFood.length; i++) {
		//lastFood[i]
	...
	var currentSevenDays = GetDates(),
		withoutNext = FilterWithoutNext(json.foods),
		withoutNextBySeason = FilterBySeason(withoutNext),
		needsUpdate = false;
		
		
	
	for (let i = 4; i < currentSevenDays.length; i++) {
		var currentDay = RemoveWeekDayFromString( currentSevenDays[i] ),
			nextFood = FilterByNext(tempJson, currentDay);
			

		if ( !nextFood.length ) {
			
			
			var	randomFood = Math.floor(Math.random()*withoutNextBySeason.length);			
			
			if( !withoutNextBySeason[randomFood] ) continue;
			
			var index = json.foods.map(function (obj) { return obj.name; }).indexOf(withoutNextBySeason[randomFood].name);
			
			console.log( currentDay );
			console.log( withoutNextBySeason[randomFood].name );
			
			json.foods[index].next = currentDay;
			needsUpdate = true;
			
			withoutNextBySeason.splice(randomFood, 1 );
...
function UploadJSON () {
	...
	var settings = {
	  "url": 'https://getpantry.cloud/apiv1/pantry/'+pantryKey+'/basket/'+pantryBasket+'',
	  "method": "PUT",
	  "timeout": 0,
	  "headers": {
...
function GetJSON () {
	...
	$.ajax(settings).done(function (response) {
	  ...
	  foods = json.foods;
	  GenerateFood();
	  FillFoodListWithFood();
	  FillCalenderWithFood();
	  LazyLoadImages('.food-wrapper img[data-src], .section[data-cat="foods"] img[data-src]:visible');
	  $('#preloader').fadeOut();
	});
...
/*
 * Fill calender with food
 */
function FillCalenderWithFood() {
	var startDate = new Date(),
		currentSevenDays = GetDates(),
		calender = $("#mini-calender"),
		foodWrapper = $(".food-wrapper"),
		foodList = $('.section[data-cat="foods"] .food-list');

	for (let i = 0; i < currentSevenDays.length; i++) {

		var currentDay = currentSevenDays[i],
			weekArray = currentDay.split('.'),
			weekday = weekArray[0],
			day = weekArray[1],
			month = weekArray[2],
			today = (weekArray[1] == startDate.getDate())? " today active": "",
			previousDay = (i < 3)? " prev": "",
			nextDay = (i > 3)? " next": "";

			var foodByDate = $('<div data-weekday="'+weekday+'" class="food-by-date'+today+previousDay+nextDay+'"><div class="selected-date"></div><div class="food-list empty"><img src="images/shine-young-woman-meditating-in-nature-surrounded-by-butterflies-1.png"><div class="selected">Preskočen ručak na ovaj dan.</div></div></div>').appendTo(foodWrapper),
	...
	//			console.log( JSON.stringify(foods) );
				...
				var nextFood = FilterByNext(foods, currentDay);
				
				if( nextFood.length ) {

					foodList.removeClass('empty').html('');


					for (let r = 0; r < nextFood.length; r++) {
						var food = nextFood[r];

						foodList.append(CreateFoodHtmlElement(food.cat, food.icon, food.name, food.rating, false));
					}

				} else {
					foodList.removeClass('empty').html('Wee Need Food');
				}

			// For each prev or today
			} else {

				var lastFood = FilterByLast(foods, currentDay);

				if( lastFood.length ) {

					foodList.removeClass('empty').html('');

					for (let r = 0; r < lastFood.length; r++) {
						var food = lastFood[r];
		...
		calender.append('<div data-weekday="'+weekday+'" class="date'+today+previousDay+nextDay+'"><div class="date-wrap"><span class="week">'+weekday.slice(0,3)+'</span><span class="day">'+day+'</span></div></div>');
	}
}
...
for (let i = 0; i < icons.length; i++) {
	$('.icon-container').append('<div class="icon"><img data-src="'+iconUrl+icons[i]+'"></div>');
}
*/
...
$('.section[data-cat="foods"] .food-list, .icon-container').scroll( function() {
	LazyLoadImages('.food-wrapper img[data-src], .section[data-cat="foods"] img[data-src]:visible, .icon-container img[data-src]:visible');
});
